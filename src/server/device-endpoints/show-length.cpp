void InitShowMapping() {
     svr.Post(R"(/show-length/(\w+))", [](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "http://localhost:3000");
         //check to see if the user passed in a index
        //get the name param from the endpoint
        std::smatch matches;
        std::regex_match(req.path, matches, std::regex(R"(/show-length/(\w+))"));
